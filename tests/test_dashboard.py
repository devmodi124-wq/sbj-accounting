...
def _seed(client):
    ...
    client.post("/api/orders", json={
        "customer_name": "Cust A", "order_date": TODAY, "item_name": "Ring", "status": "delivered",
        "payment_received": "10000", "items": [{"component_type_id": comps[0], "price": "40000"}],
    })
    # Order 2: total 20000, received 20000 -> balance 0 (customer B), pending status
    client.post("/api/orders", json={
        "customer_name": "Cust B", "order_date": TODAY, "item_name": "Chain", "status": "pending",
        "payment_received": "20000", "items": [{"component_type_id": comps[1], "price": "20000"}],
    })
