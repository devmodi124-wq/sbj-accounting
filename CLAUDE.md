...
- **Void / delete**: `orders.is_cancelled` is a soft void (`POST /api/orders/{id}/cancel`, any user, reversible) — a cancelled order keeps its data + audit trail but is **excluded from every money aggregation** (sales/stock/debtors/customer reports, dashboard, customer ledger) and its cash mirror is removed; it still appears in the Sales report (greyed, excluded from totals) so it can be restored. Hard `DELETE /api/orders/{id}` is admin-only and also removes the mirrored cash entry. When threading new order aggregations, remember to add `Order.is_cancelled.is_(False)`.
